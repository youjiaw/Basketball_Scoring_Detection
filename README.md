...
X. B. Fu等人利用YOLO模型判斷出籃筐位置，並利用圖像差分找出移動中的物體，最後依照此物體是否有通過籃筐位置來判斷是否進球[2]。此方法的優點在於，只要籃球有通過籃筐範圍就會被判斷成有進球，所以進球一定會被判斷出來，但是一樣無法判斷籃球是從籃筐內部還是外部通過，所以可能會造成籃球與籃筐範圍重疊但未進入籃筐卻被誤判成進球的情況。  
...
C. L. Huang等人利用SVM(Support Vector Machine)來辨識籃球與籃筐，並將籃球依照是否被籃網覆蓋區分為在籃筐內、在籃筐外兩種類別[3]。此方法雖然可以在籃球和籃筐重疊時分辨出籃球是否在籃筐內，但因為過程中每一幀畫面都要判斷SVM的分數以確定籃球和籃筐的位置，故處理影片的速度較慢。  
